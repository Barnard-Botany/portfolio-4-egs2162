...
```{r}
# phylocom is a demonstration dataset 

data(phylocom)
names(phylocom)
head(phylocom) # genertes information output in the console 
```
...
phy <- phylocom$phylo
comm <- phylocom$sample # abundance matrix
traits <- phylocom$traits

phy

#Now let's prune phy so that its taxa exactly match the taxa in comm
prunedphy <- prune.sample(comm, phy) # cutting so community data matches our list (phy)
prunedphy
plot(prunedphy) #plotting the cleaned data

# branch lengths all the same length in the output 
```

Once that's done, we can do something right away to make a point about what we're doing. 

```{r}
# going to plot our data 6 times - each time with clumps (from above) plotted into our data 
...
# trees with yellow markers/labels on tips - marking the clumps defined above 
# in the loop: if present in cluster, plot a dot (labled)
# in these graphs we can see how speices (from our communities) fit into clades
```


```{r}
pd.result <- pd(comm, phy, include.root=TRUE) # pd as phylogenetic diversity/distance
pd.result
```
...
# here we plot traits instead of presence of a species (in a community)
# so colored tips show traits (to help compare how traits do/dont evolve in endspeices)
...
plot(tr_one_lengths, type="cladogram", direction="upwards")
```
...
# prunedAB as the info for the tree // abuncance as the matrix
...
# recomputes all things but looks at them nonindependently (to avoid bias)
...
# import data
IndIngredientsFile = read.csv("CandyListS.csv") 

# make species lists/occurance data 
snickers.occ = unique((IndIngredientsFile$Snickers))
reeses.occ = unique((IndIngredientsFile$Reeses))
hershey.occ = unique((IndIngredientsFile$Hershey))
gummmyBear.occ = unique((IndIngredientsFile$GummyBear))
jollyRancher.occ = unique((IndIngredientsFile$JollyRancher))
kitKat.occ = unique((IndIngredientsFile$KitKat))

# convert to abundance matrix 
snickers.ab = count(snickers.occ)
reeses.ab = count(reeses.occ)
hershey.ab = count(hershey.occ)
gummyBear.ab = count(gummmyBear.occ)
jollyRancher.ab = count(jollyRancher.occ)
kitKat.ab = count(kitKat.occ)

# format into a table 
snickers.ab = data.frame("SNICKERS", snickers.ab) # adding an identifier
names(snickers.ab)<-c("Candy", "SpeciesID", "Freq")

reeses.ab = data.frame("REESES", reeses.ab)
names(reeses.ab)<-c("Candy", "SpeciesID", "Freq")

hershey.ab = data.frame("HERSHEY", hershey.ab)
names(hershey.ab)<-c("Candy", "SpeciesID", "Freq")

gummyBear.ab = data.frame("GUMMMY", gummyBear.ab)
names(gummyBear.ab)<-c("Candy", "SpeciesID", "Freq")

jollyRancher.ab = data.frame("JOLLY", jollyRancher.ab)
names(jollyRancher.ab)<-c("Candy", "SpeciesID", "Freq")

kitKat.ab = data.frame("KITKAT", kitKat.ab)
names(kitKat.ab)<-c("Candy", "SpeciesID", "Freq")

# combine 6 lists
allIngredients=rbind(snickers.ab, reeses.ab, hershey.ab, gummyBear.ab, jollyRancher.ab, kitKat.ab)
dim(allIngredients)

# matrify
Ingredients <- matrify(allIngredients)
Ingredients

# use taxize to resolve the master list for "proper taxonomy" [code dosent work]
for (i in colnames(Ingredients))
{
  #print(i) # print check on for loop object
  tsn=get_tsn(i, accepted=FALSE) # find tsn
  #tsn
  
  lapply(tsn, itis_acceptname) # apply accepted name (incase not the same)
  #print(itis_acceptname)
}


# phylogenize
SpeciesList = colnames(Ingredients) 
SpeciesList
taxa <-c(SpeciesList)
taxa

  # using stored tree zanne2014 (includes branch length)
tree_phylomatic <- phylomatic(taxa = SpeciesList, storedtree ="zanne2014", get = 'POST') 
plot(tree_phylomatic, no.margin = TRUE) 

  # save phylogenty as a file 
fileNamer = "PlainPhylo"
write.tree(tree_phylomatic, file=fileNamer)

# save image as a file (PDF)
pdf_fn = "PlainPhylo.pdf"
pdf(file=pdf_fn)
plot(tree_phylomatic, no.margin = TRUE) 
dev.off()
...
# create PDF of 6 trees 
pdf_fn_t = "TaggedPhylo.pdf"
pdf(file=pdf_fn_t)
plot(tree_phylomatic, no.margin = TRUE) 

# 6 annotations of the list 
  # create matrix (6 spots)
par(mfrow = c(3, 2))

  # create [creates the 6 trees, but they remain unlabled (see notes in for loop)]
for (i in rownames(Ingredients)) # "for each of the species a row in the matrix"
{
  plot(tree_phylomatic, show.tip.label = FALSE, main = i) # doesnt show species names
  
  # should: label the tips (ingredients) that are assoicated with one type of candy
  # should scann through all tips: tree_phylomatic$tip.label
  # and compare to all species that are listed under a single candy: names()
  
  # I've tried a variety of apporahes to solving this, but so far had no success:
  # modifying the "which" and "names command"; formatting/altering Ingredients so that its only a list of strings; 
  # I see that the first portion of the argumetn following the "=" should reference the tree (here its "tree_phylomatic") and then compare it to/find matches with ("%in%"") the list of species that are within a certain candy/row in the matrix ("names(which(Ingredients [i, ] > 0))")
  # for some reason this isnt happening though - I predict that the problem is in the second half of this, and I am not propperly referencing the list of species that belongs to a type of candy
  tiplabels(tip = which(tree_phylomatic$tip.label %in% names(which(Ingredients [i, ] > 0))), pch=19, cex=1) # adds labels [doesnt work, but no errors - just no labels]

  # tree_phylomatic as the info for the tree // Ingredients as the matrix
}

dev.off() # closes PDF file ? so that 6 induvidual trees are saved

# open up PDF [shows that the headders/titles get cut off in the created PDF - not sure how to fix that/format // however the output that shows up below looks uncrowded]
cmdstr = paste("open ", pdf_fn_t, sep="")
system(cmdstr)

# shannon, simpson, and PD
  # compare diversities between candies (ie. they're run for each type)
shannon<-diversity(Ingredients, index = "shannon") 
simpson<-diversity(Ingredients, index = "simpson") 

pd.result <- pd(Ingredients, tree_phylomatic, include.root=TRUE) # [error here: invalid 'times' argument] - not sure how to fix
  # I put in "Ingredients" as my abuncance matrix adn "tree_phylomatic" as the data for my tree, which reflects the structure for the "pd" command that is in the sample code (as I could interpret it)

# view results
shannon
simpson
pd.result # [doesnt have a value because of the error in creating it]

# compare diversity indicies 
# the simpson and shannon indicies together indicate that between candy types, the greatest diversity is observed in the list of gummy bear ingredients. However, the biodiveristy in all is very similar, with values that are very close in size. This may be due to the fact taht 1) the lists are not all long, and 2) the ingredientes were very similar
# when doing this analysis, I was shocked to find the similarities in lists of ingreidents - it was as if all candies (espeicaly ones that include chocolate) are all just different versions of the same 5-10 ingredients: sugar, chocolate, dairy, palm oil, some emulsifier (ususally soy), and sometimes peanuts
# I was unable to get the pd index to run (couldnt figure out how to format my abuncance matrix such that I could put it into the pd function with my phylogeny). However, as a result of the very similar lists of ingredients, and thus a shorter list of indivudial plant species, I recognize that my tree has relatively few branches (as compared to one compiled from 6 recipies/products with very different ingredients). In these 6 products, there are only 13 unique ingredients (thats crazy!)

# additional analysis from today's lab: tag tips of a specific classification 
taggedList = ("Arachis hypogaea")

for (i in colnames(Ingredients))
{
  name = i
  tsn=get_tsn(i, accepted=FALSE) # obtain tsn 

  # here I would add the name of the present species to "taggedList" if it met the chriteria that I was looking for
  
  # then when (out of the for loop) I plotted the tree with the labeled tips, the ones with the chriteria would be labeled
  
  # ran out of time to do (also was having trouble with getting labels to show up in the first place )
  
}
 
# plot tree with specified tips lableled (and fun labels)
plot(tree_phylomatic, show.tip.label = FALSE, main = i) # doesnt show species names
tiplabels(tip = which(tree_phylomatic$tip.label %in% taggedList))
tiplabels(pch = 12, col = "blue", cex = 2) # labels all of them 

# NOTE: 
# unfortunately, I wasn't able to successfully finsih all the parts of this mini-excersise. I'm hoping to go into office hours to resolve the issues I was having, but too left notes about what I was seeing versus what I was wishing to see in the hopes that I could get some feedback on what may be the problem. 
